...
try:
    ...
    from metrics.bleu_script import compute_bleu
except ImportError:
    from src.metrics import Metric, ConfigurationError
    from src.metrics.bleu_script import compute_bleu
...
class BLEU(Metric):
    ...
    def __init__(
        ...
        exclude_indices: Set[int] = {0}, # TODO this does not work for all transformer models
    ) -> None:
        self._ngram = 4
    ...
    def __call__(
        self,  # type: ignore
        predictions: Union[torch.LongTensor, List],
        gold_targets: Union[torch.LongTensor, List],
        mask = None,
        ...
        """
        if input_type == 'class':
            #assert input_type == 'class', "Wrong tensor input type for BLEU computation!"
            self._input_type = 'class'
            ...
            for ngram_size in range(1, self._ngram + 1):
            ...
            predictions, references = self._format_pred_ref(predictions, gold_targets)
            bleu_n, precisions, brevity_penalty, ratio, _prediction_lengths, _reference_lengths = compute_bleu(references,
                                                                                predictions, max_order=4, smooth=False)
            self._bleu_n = bleu_n

    def _format_pred_ref(self, predictions, gold_targets):
        """
        Format predictions and targets as required by func::compute_bleu()
        :param predictions:
        :param gold_targets:
        :return:
        """
        # todo Assume we only have one gold reference

        formatted_preds = [pred.split() for pred in predictions]
        formatted_refs = [[target.split()] for target in gold_targets]
    ...
    def get_metric(self, reset: bool = False):
        ...
        if self._input_type == 'class':
            ...
            bleu_n = {f'BLEU-{i}': brevity_penalty * math.exp(sum(1/i * score for score in ngram_scores[:i]))
                            for i in range(1, self._ngram + 1)}

            output_bleu = bleu_n
        elif self._input_type == 'string':
            output_bleu = self._bleu_n
        else:
            raise ValueError("BLEU only accept input types as `class` or `string`")
        if reset:
            self.reset()
    ...
    def all_metric_names(self):
        return [f'BLEU-{i}' for i in range(1, self._ngram + 1)]