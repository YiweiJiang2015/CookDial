...
try:
    from metrics import Metric, ConfigurationError, FBetaMeasure
except ImportError:
    from src.metrics import Metric, ConfigurationError, FBetaMeasure
...
class FBetaMultiLabelMeasure(FBetaMeasure):
    ...
    threshold: `float`, optional (default = `0.5`)
        Probability over this threshold will be considered predictions for the corresponding class.
    ...
    def __init__(
        ...
        super().__init__(beta, average, labels, label_space)
    ...
    def __call__(
        ...
        if input_type == 'logit':
            predictions = torch.sigmoid(predictions)  # for each class dim, we do a binary classification
        ...
        true_positives = (gold_labels * threshold_predictions).bool() & mask & pred_mask # difference from single-label computation
...
class F1MultiLabelMeasure(FBetaMultiLabelMeasure):
    """
    For the intent prediction in Task-I: user question understanding
    """
    def __init__(
        self, average: str = 'micro', labels: List[int] = None, threshold: float = 0.5, label_space: dict = None) -> None:
        super().__init__(1.0, average, labels, threshold, label_space)