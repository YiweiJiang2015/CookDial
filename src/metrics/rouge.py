...
try:
    ...
    from metrics.rouge_script import compute_rouge
    from utils.allennlp_util import ngrams, get_valid_tokens_mask
except ImportError:
    from src.metrics import Metric, ConfigurationError
    from src.metrics.rouge_script import compute_rouge
    from src.utils.allennlp_util import ngrams, get_valid_tokens_mask
...
class ROUGE(Metric):
    ...
    def __call__(
        ...
        if input_type == 'class':
            ...
            rouge_scores = compute_rouge(predictions, gold_targets)
            self._rouge_scores = rouge_scores
    ...
    def get_metric(self, reset: bool = False) -> Dict[str, float]:
        ...
        if self._input_type == 'class':
            ...
            output_metrics = {f"ROUGE-{i}": metrics[f"ROUGE-{i}_F1"] for i in range(1, self._ngram_size + 1)}
            output_metrics["ROUGE-L"] = metrics["ROUGE-L"]

        elif self._input_type == 'string':
            output_metrics = self._rouge_scores
        else:
            raise ValueError("ROUGE only accept input types as `class` or `string`")
        if reset:
            self.reset()
        return output_metrics

    def all_metric_names(self) -> list[str]:
        return [f"ROUGE-{i}" for i in range(1, self._ngram_size+1)] + ["ROUGE-L"]